...
def user_has_valid_snap(client, parent_author, parent_permlink, username):
    ...
    logger.info(f"user_has_valid_snap: get_content({parent_author}, {parent_permlink}) returned: {post}")
    if not post:
        logger.info(f"user_has_valid_snap: No post found for author={parent_author}, permlink={parent_permlink}")
        return False
    ...
    for b in beneficiaries:
        ...
        if b.get('account') == 'snapnpay' and b.get('weight') == 5000:
            logger.info(f"user_has_valid_snap: Valid snap detected for author={parent_author} with snapnpay beneficiary.")
            found = True
            break
    if not found:
        logger.info(f"user_has_valid_snap: No valid snapnpay beneficiary found. Full post: {post}")
    return found